...
class Parser():
    ...
    def options(self, text: str) -> dict[str, str]:
        ...
        Raises:
            ValueError: If any block is not a valid key=value pair,
                or if a key is empty or duplicated.
        """
        values: dict[str, str] = {}
        ...
        for block in blocks:
            if "=" not in block:
                raise ValueError(
                    f"Metadato mal formado (falta '='): '{block}'"
                )
            ...
            if not key_b:
                raise ValueError(
                    f"Metadato con clave vacía: '{block}'"
                )
            if not value_b:
                raise ValueError(
                    f"Metadato con valor vacío: '{block}'"
                )
            if key_b in values:
                raise ValueError(
                    f"Clave de metadato duplicada: '{key_b}'"
                )

            values[key_b] = value_b
        return values

    def parser(self) -> Map | None:
        ...
        try:
                    ...
                    if ":" in line:
                        ...
                        if key == "connection":
                            ...
                            if after:
                                ...
                                optional_values = self.options(inside)
                            ...
                            if "-" in zone_name:
                                raise ValueError("El nombre de la zona no "
                                                 f"puede contener guiones: '"
                                                 f"{zone_name}'")
                            if " " in zone_name:
                                raise ValueError("El nombre de la zona no "
                                                 f"puede contener espacios: '"
                                                 f"{zone_name}'")
                            try:
                            ...
                            if after:
                                ...
                                if ("max_drones" in optional_values and
                                        zone_type not in ("start_hub",
                                                          "end_hub")):
                                    max_drones = int(
                                        optional_values["max_drones"])
                                    if max_drones <= 0:
                                        raise ValueError(
            ...
            if nb_drones <= 0:
                raise ValueError("El número de drones debe ser "
                                 "mayor que cero.")
            ...
            map_obj.min_x = min(
                z.coordinates[0] for z in map_obj.zones.values())

            map_obj.max_x = max(
                z.coordinates[0] for z in map_obj.zones.values())

            map_obj.min_y = min(
                z.coordinates[1] for z in map_obj.zones.values())
            ...
            print(f"ERROR: Sin permisos para leer '{self.filepath}'.")
            return None
